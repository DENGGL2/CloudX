...
class RemoteNotificationManager(private val context: Context) {
    ...
    fun preview(mode: TaskNotificationMode): RemoteNotificationResult {
        ...
        if (shouldRequestPostNotificationPermission()) {
            return RemoteNotificationResult(
                ...
                message = "缺少通知权限，请在系统设置中允许 cloud code 发送通知",
            )
        }
        if (!notificationManager.areNotificationsEnabled()) {
            return RemoteNotificationResult(
                sent = false,
                message = "系统通知已关闭，请在系统设置中允许 cloud code 发送通知",
            )
        }
        ...
        val previewText = "cloud code 会在电脑端任务状态变化时通知你"
            ...
            mainHandler.postAtTime(
                {
                    post(
                        ...
                        title = "岛通知已选择",
                        text = "cloud code 会在电脑端任务状态变化时通知你",
                        liveUpdate = true,
                        progress = 100,
                        shortText = "完成",
                        final = true,
    ...
    fun notifyTaskEvent(
        ...
        val (title, text, progress, shortText, final) = when (event.kind) {
            RemoteTaskNotificationEvent.Kind.RUNNING -> NotificationContent(
                title = "cloud code 正在处理任务",
                text = "${event.conversationTitle} 正在电脑端执行，可点按返回 cloud code 查看进度",
                progress = 5,
                shortText = "执行中",
                final = false,
            )
            RemoteTaskNotificationEvent.Kind.WAITING_FOR_PERMISSION -> NotificationContent(
                title = "cloud code 需要你的确认",
            ...
            RemoteTaskNotificationEvent.Kind.FAILED -> NotificationContent(
                title = "cloud code 任务失败",
                text = event.detail ?: "${event.conversationTitle} 未能完成，可返回 cloud code 查看原因",
                progress = 100,
                shortText = "失败",
                final = true,
            )
    ...
    private fun createChannels() {
        ...
        // Android keeps channel records across upgrades. Remove every channel
        // created by former Mason-branded builds, including renamed variants.
        ...
        notificationManager.createNotificationChannel(
            ...
            ).apply { description = "cloud code 发送的任务状态通知" },
        )
        notificationManager.createNotificationChannel(
            NotificationChannel(
    ...
    companion object {
        const val CHANNEL_ID = "cloud_code_tool_notification"
        const val LIVE_UPDATE_CHANNEL_ID = "cloud_code_task_live_update"
        const val CHANNEL_NAME = "cloud code 工具通知"
